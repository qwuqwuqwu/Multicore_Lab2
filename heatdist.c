 ...
 * such as memory allocation, data movement, etc. 
 */
...
int main(int argc, char * argv[])
{
  ...
  switch(which_code)
  {
	...
	case 1: printf("OpenMP version:\n");
			start = omp_get_wtime();
			parallel_heat_dist(playground, N, iterations, numthreads); 
			end = omp_get_wtime();  
...
*/
void  parallel_heat_dist(float * playground, unsigned int N, unsigned int iterations, unsigned int numthreads)
{
  // Loop indices
  int i, j, k;
  int upper = N-1; //used instead of N to avoid updating the border points
